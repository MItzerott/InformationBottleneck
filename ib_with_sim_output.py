...
if args.configname:
    ...
    config_filename = 'config_sim.toml'
    
local_config.config_filename = config_filename

from IB_refined import IB_refined
from data_generation_refined import snapshot_to_data
import time
...
def main(mode, smoothing_scale, output = False):
    ...
    ds = snapshot_to_data(
        plot = False, 
        all = bool(load_config(filename = config_filename)['sim_params']['load_all'] == 1),
        size = int(load_config(filename = config_filename)['sim_params']['sim_size']),
        identifier = str(load_config(filename = config_filename)['sim_params']['sim_identifier'])
    )
    ...
    #fit_param = pd.DataFrame(data={'alpha': [1, 1, 1, 1], 'p0': [0.0, 1.0, 0.5, 0.0], 'waviness': [1.0, 1.0, 0.0, 0.0]})
    ...
    fit_param['Tmax'] = load_config(filename = config_filename)['ib_fit_params']['Tmax']
    # fit models

    metrics_conv, dist_conv, metrics_sw, dist_sw = IB_refined(ds, fit_param, conv_dist_to_keep = {'qt_x','qt'}, sw_dist_to_keep = False, quiet = True, output = output)
...
end_time = time.time()
print('Algorithm ran for %.2fs' %(end_time-start_time), flush = True)
